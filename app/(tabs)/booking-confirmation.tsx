...
export default function BookingConfirmation() {
  ...
  const selectedTime = params.selectedTime as string || "10:00";
  const selectedDate = params.selectedDate as string || "2026-03-12";
  const tutorName = params.tutorName as string || "Mentor";
  const tutorPhone = params.tutorPhone as string || "";

  // Debug: Log what we received
  console.log('🕐 Booking Confirmation - Received params:', {
    bookingId,
    selectedTime,
    selectedDate,
    tutorName,
  ...
  // Debug: Log subjects specifically
  console.log('🔍 Subjects Debug:', {
    subjects: params.subjects,
    subjectsType: typeof params.subjects,
    subjectsString: String(params.subjects),
    subjectsStringified: JSON.stringify(params.subjects)
  });
  ...
  const [teacherLocation, setTeacherLocation] = useState({
    ...
    eta: 15, // minutes
    distance: 2.5, // km
    phone: tutorPhone || "+919876543210"
  });

  // Handle real phone call
  const handleCallMentor = () => {
    if (teacherLocation.phone) {
      Alert.alert(
        "Call Mentor",
        `Would you like to call ${teacherLocation.name} at ${teacherLocation.phone}?`,
        [
          {
            text: "Cancel",
            style: "cancel"
          },
          {
            text: "Call",
            onPress: () => {
              // Open phone dialer with mentor's number
              Linking.openURL(`tel:${teacherLocation.phone}`).catch((err: any) => {
      ...
      Alert.alert("No Phone Number", "Mentor phone number not available.");
    }
  };

  // Calculate ETA based on distance and current time
  const calculateETA = (distanceKm: number, bookedTime: string) => {
    // Assuming 30 km/h average speed in city traffic
    const travelTimeMinutes = Math.ceil((distanceKm / 30) * 60);
    
    // Parse booked time (handle both 24-hour and 12-hour formats)
    let hours, minutes;
    if (bookedTime.includes("AM") || bookedTime.includes("PM")) {
      const timePart = bookedTime.split(" ")[0];
      [hours, minutes] = timePart.split(":").map(Number);
      const period = bookedTime.split(" ")[1];
      
      if (period === "PM" && hours !== 12) hours += 12;
      if (period === "AM" && hours === 12) hours = 0;
    } else {
      [hours, minutes] = bookedTime.split(":").map(Number);
    }
    
    // Create Date object for today with booked time
    const etaDate = new Date();
    etaDate.setHours(hours, minutes + travelTimeMinutes, 0, 0);
    
    // Format ETA time in 24-hour format
    const etaHours = etaDate.getHours();
    const etaMinutes = etaDate.getMinutes();
    
    return {
      etaMinutes: travelTimeMinutes,
      etaTime: `${etaHours.toString().padStart(2, '0')}:${etaMinutes.toString().padStart(2, '0')}`
    };
  };
  ...
  const formattedBookedTime = formatTime(selectedTime);
  ...
  // Get current ETA based on distance
  const currentETA = calculateETA(teacherLocation.distance, selectedTime);
  ...
  // Send location notification to parent
  const sendLocationNotification = (distance: number, eta: number) => {
    const notificationMessage = `📍 Mentor Update: ${teacherLocation.name} is ${distance.toFixed(1)} km away. ETA: ${eta} minutes.`;
    
    // In real app, this would send push notification
    console.log('🔔 Location Notification:', notificationMessage);
    
    // Show as alert for demo
    if (distance < 1.0) {
      Alert.alert(
        "📍 Mentor Nearby!", 
        `${teacherLocation.name} is just ${distance.toFixed(1)} km away! ETA: ${eta} minutes.`,
        [{ text: "OK" }]
      );
    }
  };
  ...
  useEffect(() => {
    // Use real booking code from backend, or generate fallback if not available
    const code = bookingCode || Math.random().toString(36).substr(2, 8).toUpperCase();
    setConfirmationCode(code);
    console.log('🎯 Booking Code Set:', code);

    // Send booking data to Supabase
    const sendBookingToSupabase = async () => {
      try {
        // Debug: Parse subjects safely
        let parsedSubjects = {};
        if (params.subjects === "[object Object]") {
          parsedSubjects = {};
          console.log('🔍 Subjects is [object Object], using empty object');
        } else if (typeof params.subjects === 'string') {
          try {
            parsedSubjects = JSON.parse(params.subjects || '{}');
            console.log('🔍 Successfully parsed subjects from string');
          } catch (e) {
            parsedSubjects = {};
            console.log('🔍 Failed to parse subjects, using empty object:', e);
          }
        } else {
          parsedSubjects = params.subjects || {};
          console.log('🔍 Using subjects as-is');
        }
        ...
        console.log('🔍 Confirmation Page - Sending booking data to Supabase:', JSON.stringify(bookingData, null, 2));

        const response = await fetch('http://192.168.0.25:8081/api/payment/create-booking', {
          method: 'POST',
        ...
        console.log('✅ Supabase booking result:', result);

        if (result.success) {
          console.log('✅ Booking stored in Supabase successfully!');
        } else {
          console.log('❌ Failed to store booking in Supabase:', result.error);
        }
      } catch (error) {
        console.error('❌ Error sending booking to Supabase:', error);
      }
    };

    // Call the backend to store booking data
    ...
    const locationInterval = setInterval(() => {
      setTeacherLocation((prev) => {
        ...
        // Calculate new position (move 10% closer)
        const newLat = prev.latitude + latDiff * 0.1;
        const newLng = prev.longitude + lngDiff * 0.1;
        
        // Calculate new distance
        const newDistance = calculateDistance(newLat, newLng, destinationLocation.latitude, destinationLocation.longitude);
        
        // Calculate new ETA based on distance
        const newEta = calculateETA(newDistance, selectedTime);
        
        // Send notification for significant updates
        if (Math.floor(prev.distance) !== Math.floor(newDistance)) {
          sendLocationNotification(newDistance, newEta.etaMinutes);
        }
      ...
      <SafeAreaView style={styles.topHeaderSafeArea}>
        <View style={styles.topHeader}>
          ...
          <View style={styles.headerTextWrap}>
            ...
            <Text style={styles.headerTitle}>{teacherLocation.distance.toFixed(1)} km away • {teacherLocation.eta} min</Text>
          </View>
          <View style={{ width: 28 }} /> 
        </View>
      </SafeAreaView>
      ...
      >
        ...
        <View style={styles.mapBox}>
          ...
          <View style={styles.distanceBadge}>
            <Feather name="navigation" size={14} color={PAL.PURE_WHITE} />
            <Text style={styles.distanceText}>{teacherLocation.distance.toFixed(1)} km</Text>
          </View>
        ...
        >
          ...
          <View style={styles.bpRow}>
            <View>
              <Text style={styles.bpTime}>{formattedBookedTime}</Text>
            ...
            <View style={{ alignItems: "flex-end" }}>
              <Text style={styles.bpTime}>{currentETA.etaTime}</Text>
        ...
        <View style={styles.actionRow}>
          <Pressable style={styles.callBtn} onPress={handleCallMentor}>
